...
#define E1000_SPDMDEV_MAX_BUF 0x1000 //4096
...
static size_t e1000_spdm_buf_size;
...
/* spdm functions */
libspdm_return_t e1000_spdm_send (
  void                    *spdm_context,
  size_t                   request_size,
  const void              *request,
  uint64_t                 timeout
  );
libspdm_return_t e1000_spdm_receive (
  void                    *spdm_context,
  size_t                  *response_size,
  void                   **response,
  uint64_t                 timeout
  );
libspdm_return_t e1000_spdm_get_response_vendor_defined_request(
  ...
  )
{
    ...
    if (request_size > sizeof(e1000_spdm_buf)) {
        printf("request_size too large %lu\n", request_size);
        return LIBSPDM_STATUS_BUFFER_TOO_SMALL;
    }

    struct iovec iov = {
        .iov_base = (char *) request,
        .iov_len = request_size,
    };
  ...
  )
{
    ...
    if (*response_size < atomic_read(&e1000_spdm_buf_size)) {
    ...
    *response_size = e1000_spdm_buf_size;
    memcpy(*response, e1000_spdm_buf, *response_size);
    qemu_mutex_unlock(&e1000_spdm_io_mutex);
    return LIBSPDM_STATUS_SUCCESS;
}

void e1000_spdm_server_callback (void* spdm_context)
{
    static bool algo_provisioned = false;
    bool res;
    ...
    // algo_provisioned = true;
    atomic_set(&algo_provisioned, true);
...
libspdm_return_t e1000_acquire_buffer (void *context, void **msg_buf_ptr);
void e1000_release_buffer(void *context, const void *msg_buf_ptr);

libspdm_return_t e1000_acquire_buffer (void *context, void **msg_buf_ptr){
    *msg_buf_ptr = (void *)malloc(E1000_SPDMDEV_MAX_BUF);
    if (*msg_buf_ptr == NULL)
...
static int e1000_spdm_init(E1000State *s) {
    ...
    E1000_SPDM_PRINT("init_spdm\n");
    s->spdm_context = (void *)malloc(libspdm_get_context_size());
    ...
    libspdm_register_transport_layer_func(
            s->spdm_context,
            (E1000_SPDMDEV_MAX_BUF - LIBSPDM_MCTP_TRANSPORT_HEADER_SIZE - LIBSPDM_MCTP_TRANSPORT_TAIL_SIZE),
    ...
    libspdm_register_device_buffer_func(
        s->spdm_context,
        E1000_SPDMDEV_MAX_BUF,
        E1000_SPDMDEV_MAX_BUF,
        e1000_acquire_buffer,
        e1000_release_buffer,
        e1000_acquire_buffer,
        e1000_release_buffer);
    ...
    libspdm_set_scratch_buffer(s->spdm_context, scratch_buffer, scratch_buffer_size);
    ...
    data32 = e1000_m_use_responder_capability_flags;
    ...
    libspdm_register_get_response_func(s->spdm_context, e1000_spdm_get_response_vendor_defined_request);
...
process_tx_desc(E1000State *s, struct e1000_tx_desc *dp)
{
    ...
    demo_e1000_print_buffer((char*) tp->data, tp->size, "Network card received the following data from OS:");
#if E1000_SPDM_DEBUG
    //Aqui vem o tratamento das mensagens SPDM
    int i;
    printf("[QEMU] tp->size: %02X\n", tp->size);
    for(i = 0; i < tp->size; i++)
        printf("[QEMU] tp->data: %02X ", tp->data[i]);// , tp->data[i]);
#endif
    ...
    else
    {
        ...
        if (!(tp->cptse && tp->size < tp->tso_props.hdr_len)) {
            ...
            scratch_buffer = scratch_buffer + libspdm_get_scratch_buffer_secure_message_offset(s->spdm_context) +
                transport_header_size;
            scratch_buffer_size = libspdm_get_scratch_buffer_secure_message_capacity(s->spdm_context) -
                transport_header_size;
#else
            ...
            // size_t temp_size = tp->size;

            libspdm_return_t status = ((libspdm_context_t *)s->spdm_context)->transport_decode_message(
                    s->spdm_context,
                    &session_id,
                    &is_app,
                    TRUE,
            ...
            tp->size = scratch_buffer_size; // cast de 32 pra 16 bits
            memcpy(tp->data, scratch_buffer, tp->size);

            if (LIBSPDM_STATUS_IS_ERROR(status))
                printf("[QEMU] spdm_transport_mctp_decode_message fail - status: %x\n", status);
            ...
            if (desc_offset < size) {
                ...
                do {
                    ...
                    if (!spdm_msg_type)
                    {
                        ...
                        {
                            size_t i; //, j;
                            printf("[QEMU] packet_len: %ld\n[QEMU] packet:", iov_copy);
                            ...
                            for (i = 0; i < iov_copy; ++i) {
                                if (i%16 == 0)
                                    printf("\t[QEMU] packet[%ld] ", i) ;
                                printf("%02x ", ((uint8_t*)iov->iov_base)[i]);
                            }
                            printf("\n");
                        }
#endif
                        ...
                        void* scratch_buffer;
                        size_t scratch_buffer_size;
                        uint32_t session_id = (((libspdm_context_t *)s->spdm_context)->last_spdm_request_session_id); //0xFFFFFFFF;
                        ...
                        if (iov_copy > scratch_buffer_size) {
                            printf("iov_copy > scratch_buffer_size\n");
                            return -1;
                        }
                        scratch_buffer_size = iov_copy;
                        memcpy(scratch_buffer, iov->iov_base + iov_ofs, iov_copy);

                        e1000_spdm_buf_size = E1000_SPDMDEV_MAX_BUF;
                        void *passing_ptr = malloc(e1000_spdm_buf_size);// e1000_spdm_buf;

                        libspdm_return_t status = ((libspdm_context_t *)s->spdm_context)->transport_encode_message(
                                s->spdm_context,
                                &session_id,
                                TRUE,
                                FALSE,
                                scratch_buffer_size, //iov_copy,
                                scratch_buffer, //iov->iov_base + iov_ofs,
                                &e1000_spdm_buf_size,
                                &passing_ptr
                        );
                        ...
                        if (LIBSPDM_STATUS_IS_ERROR(status))
                        {
                            printf("[QEMU] iov encoding failed - status: %x\n", status);
                        }
                        else
                        {
                            //uint32_t it;
                        ...
                        e1000_spdm_demo_post_packet(passing_ptr, e1000_spdm_buf_size);
                        // demo_e1000_print_buffer(e1000_spdm_buf, e1000_spdm_buf_size, "Network card to OS");
                        pci_dma_write(d, ba, passing_ptr, e1000_spdm_buf_size);
...
#if E1000_SPDM_DEBUG
    size_t i;
    printf("[QEMU] e1000_receive! size: %ld \n", size);
    for (i = 0; i < size; ++i)
        printf("[QEMU] rx_data[%ld]: %02X %c", i, buf[i], buf[i]);
#endif
